...
internal object StandardParser : JSONParser {
	...
	private fun parse(source: JSONReader, expectedType: ExpectedType, closeSource: Boolean) =
		if (closeSource) {
			source.use {
	...
	override fun parseValueOrNull(source: JSONReader, closeSource: Boolean) =
		parse(source, ExpectedType.any, closeSource = closeSource)


	override fun parseList(source: JSONReader, closeSource: Boolean) =
		parse(source, ExpectedType.list, closeSource = closeSource) as List<*>


	@Suppress("UNCHECKED_CAST")
	override fun parseMap(source: JSONReader, closeSource: Boolean) =
		parse(source, ExpectedType.map, closeSource = closeSource) as Map<String, *>
